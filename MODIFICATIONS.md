# Modifications by Jennifer Buehler
...
- Substitution objects can now be applied to all the expressions
  * This includes the addition of function ``apply(Substitution)`` in all expressions
- At several places, the return type of ``clone()`` was the class type itself, which then died not support the ``Object`` interface (I believe this was a bug in the old code?). Fixed this.
- Added support for start, end and over conditions and effects in DurativeAction
- Added support for a visitor pattern to visit expressions at may places: ``Object accept(ExpVisitor, Object)``
...
- metric/MetricExp.java
  * In ``hashCode()``: the hash code was generated with ``+`` while it should be with ``^`` to be more unique.

- term/AbstractTerm.java
  * Swapped ``TypeSet`` against ``Type`` (see modifications in type/Type.java)
  * removed constructor which does not pass a type to the term, thereby making it compulsorty to specify a type.
  * make function ``setType(Type)`` public so the type can be changed from outside (TODO: not sure where I needed this
    again, maybe do a search again and see if it can get protected again).
  * In ``hashCode()``: the hash code was generated with ``+`` while it should be with ``^`` to be more unique.
  * TODO: Added function ``void replaceOnlyType(Type t)`` which can be used to set a new type (the only type),
    but it is commented out at the moment. Look into this and where it was needed before getting rid of it completely.
- term/Constant.java
  * Removed ``TypeSet`` datatype usage and now only using``Type`` according to changes in fexp/FExp.java
  * Removed constructor which does not specify a type, in accordance with changes in term/AbstractTerm.java
  * Added UNDEFINED_SYMBOL for an undefined constant
  * Added function to accept visitors: ``Object accept(ExpVisitor, Object)``
- term/Substitution.java
...
- time/AtTimedExp.java
  * In ``hashCode()``: the hash code was generated with ``+`` while it should be with ``^`` to be more unique.
- time/OverTimedExp.java
  * In ``hashCode()``: the hash code was generated with ``+`` while it should be with ``^`` to be more unique.
- time/TimedExp.java
  * Added function to accept visitors: ``Object accept(ExpVisitor, Object)``
  * In ``hashCode()``: the hash code was generated with ``+`` while it should be with ``^`` to be more unique.
- time/TimedLiteral.java
  * In ``hashCode()``: the hash code was generated with ``+`` while it should be with ``^`` to be more unique.
...
- type/Type.java
  ...
  * Added function ``setImage(String)`` which allows to change the type name
    (TODO check again if I need this anywhere at all)
  * Removed functions:
    - ``isFinal()``
    - ``getSuperTypes()``
    - ``getSubTypes()``
...
- ListExp.java
  ...
  * Added functions:
    ...
    - ``static boolean allVariablesBound(Exp e, Substitution sigma)`` (private helper) to check if all variables are bound.
        TODO: Check where this is used, it looks like it's not used anywhere in the code I imported so far,
        and as it's private, it's probably not used anywhere. Must be a remnant of old modifications.
- NotExp.java
  * Added function to accept visitors: ``Object accept(ExpVisitor, Object)``
  * In ``hashCode()``: the hash code was generated with ``+`` while it should be with ``^`` to be more unique.
- PrefExp.java
...
## List of specific changes per file in the pddl4j directory
